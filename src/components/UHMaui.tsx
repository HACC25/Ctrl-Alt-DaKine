  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const resp = await fetch(buildApiUrl('/api/generate-path'), {
        ...
        body: JSON.stringify({ major: majorLabel, campus: 'maui' })
      });
      ...
      if (json?.path && Array.isArray(json.path) && json.path.length > 0) {
        ...
        const fallback = [
          { id: 'f1', name: 'BUS 101 - Intro to Business', credits: 3, position: { x: 0, y: 0 } },
          { id: 'f2', name: 'ACC 124 - Accounting', credits: 3, position: { x: 260, y: 0 } },
          { id: 'f3', name: 'CULN 105 - Culinary Fundamentals', credits: 3, position: { x: 520, y: 0 } }
        ];
